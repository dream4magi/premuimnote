Module T
    ...
    Sub removeNotePaper(ByVal noteClass As clsAllNotes.clsNoteData)
        For Each obj As Object In aryALL_NOTES_paper

            If obj.GetType.Equals(GetType(frmNoteTemplate)) Then
                If (CType(obj, frmNoteTemplate)).readNoteData.Equals(noteClass) Then
                    aryALL_NOTES_paper.Remove(obj)
                    CType(obj, frmNoteTemplate).Dispose()
                    Return
                End If

            ElseIf obj.GetType.Equals(GetType(frmNoteDetailTemplate)) Then
                If (CType(obj, frmNoteDetailTemplate)).readNoteData.Equals(noteClass) Then
                    aryALL_NOTES_paper.Remove(obj)
                    CType(obj, frmNoteDetailTemplate).Dispose()
                    Return
                End If
            End If
        Next
